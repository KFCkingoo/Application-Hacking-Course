...
Kun se syötettiin URL-päätteeseen, se `http://127.0.0.1:5000/accounts/login/?next=/admin-console/` sivustolle.

Yritettiin oikaista sisäänkirjautuminen erilaisilla syötteillä kuten `admin-console'--`.
...
Katsoin eri tiedostoja eri hakemistosta, mutta ei päädytty minnekään.

Otin ratkaisun käyttööni ja sen olisi pitänyt etsiä `hats/` hakemistosta `views.py` ohjelma. Kyseinen ohjelma löytyy myös `accounts` hakemistosta, mutta se ei ole sama.
